...
function inverse_solve_mdp_game(mdp_game_data, ŷ; max_α=1, max_iter=100, tol=2e-3, seed=1234)
    ...
    # helper function: projection onto D
    function proj_D(C; ρ=100, p=p, n=n, m=m)
        C1 = 0.5 * (C - C')
        for i in 1:p
            C1[m*n*(i-1)+1:m*n*i, m*n*(i-1)+1:m*n*i] = zeros(m*n, m*n)
        end
    ...
    function F1(y, v, b, C)
        return hcat([-b[:, i] - sum(C[(i-1)*m*n+1,i*m*n] * y[:,j] for j in 1:p) - log.(y[:,i]) - log.(D[i]'*D[i]*y[:,i]) + (D[i] - γ*E[i])' * v[:,i] for i in 1:p]...)
    end
    ...
    for i in 1:max_iter
        ...
        # ∂F_C = [∂F1_C; ∂F2_C]
        # ∂ξ_C = -pinv(∂F_ξ) * [∂F1_C; ∂F2_C]; ∂F1_C = ∂F2_C = nothing; GC.gc()
        ...
        # while(ψ(solve_ih_mdp_game_optimized(mdp_game_data, proj_B(b-α*reshape(∇ψ_b, m*n, p)), proj_D(C-α*reshape(∇ψ_C, p*m*n, p*m*n))).y) > ψ(y) - α/2 * norm([∇ψ_b; ∇ψ_C])^2)
        #     α = α / 2
        #     @show α
        # end

        # update b, C
        b = proj_B(b - α * reshape(∇ψ_b, m*n, p))
